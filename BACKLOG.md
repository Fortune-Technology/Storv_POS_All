...
**Last updated:** 2026-05-04 (+F31 vendor cover-days, +F33 custom reports, +T8 per-report deep review)
...
| ID | Status | Item | Effort | Notes |
|---|---|---|---|---|
| F31 | `[ ]` | **Vendor-level "days of cover" setting for smart auto-orders.** Add `targetCoverDays` field to `Vendor` model (e.g. 7 for weekly-delivery vendors, 30 for monthly). The 14-factor orderEngine (S42) currently uses per-product reorder points + safety stock — extend it to also consider the vendor's preferred inventory horizon when computing suggested order quantities. Example: Coca-Cola weekly delivery → keep 10 days stock on hand (so we order before running below ~3 days buffer). Output: smarter "next order" qty in Vendor Orders page that respects the buyer's preferred per-vendor cover, not just the per-product safety stock floor. Adds a new factor (#15) to the existing 14-factor algorithm: `vendorTargetCover × avgDailyDemand` becomes a soft floor on order qty. | M | Prompt |

### Storefront pricing follow-up

| ID | Status | Item | Effort | Notes |
|---|---|---|---|---|
| F32 | `[ ]` | **Storefront pricing transform at sync time.** S71d shipped the settings UI + persistence end-to-end (24/24 smoke), but the actual markup/rounding/exclusion/margin-guard pipeline does NOT yet apply when prices flow from POS → ecom-backend → `EcomProduct.retailPrice`. Required wiring: ecom-backend's `syncRoutes.js` (the `POST /sync` upsert path that iterates ecom-enabled stores and writes EcomProduct rows) needs to look up each store's storefront `pricingConfig` and apply `computeMarketplacePrice` before writing `retailPrice`. The lookup is cross-DB (POS owns `StoreIntegration`, ecom-backend owns `EcomProduct`) — cleanest is an internal HTTP endpoint POS exposes (`GET /internal/storefront-pricing-config?storeId=X`) that ecom-backend calls + caches per sync request. Also need a parallel transform on the storefront READ path so config edits reflect immediately even before the next sync. Exclusions translate to `EcomProduct.visible = false` on the upsert. Velocity computation needs to call back to POS for Transaction history (or replicate). 88/88 unit tests + 70/70 marketplace smoke + 24/24 storefront smoke all PASS for the settings layer; this item is purely the runtime application. | M | S71d deferred — settings + UI shipped, runtime not |
