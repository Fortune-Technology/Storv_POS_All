...
export default function AccountPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setProfile(profileRes.data?.data as ProfileData);
      setOrders((ordersData as OrderSummary[]) || []);
    } catch {}
    setLoading(false);
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await axios.put(
        `${ECOM_API}/store/${storeSlug}/auth/me`,
        {
  ...
  const handleAddAddress = () => {
    setProfile(p =>
      p
        ? { ...p, addresses: [...(p.addresses || []), { label: 'Home', street: '', city: '', state: '', zip: '', isDefault: false }] }
        : p
    );
  };

  const handleRemoveAddress = (idx: number) => {
    setProfile(p => (p ? { ...p, addresses: (p.addresses || []).filter((_, i) => i !== idx) } : p));
  };

  const setAddr = (idx: number, field: keyof ProfileAddress, value: string | boolean) => {
    setProfile(p => {
      if (!p) return p;
  ...
  const handleChangePassword = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setPwError(axiosErr.response?.data?.error || 'Failed to change password');
    }
    setPwSaving(false);
  };
                ...
                <div className="acc-profile-left">
                  <div className="acc-avatar-row">
                    ...
                    <div>
                      ...
                      <div className="acc-member-since">Member since {profile.createdAt ? fmtDate(profile.createdAt) : '—'}</div>
                    </div>
                  </div>
                  ...
                  <div className="acc-form-grid">
                    <div className="acc-field">
                      ...
                      <input className="acc-input" value={profile.firstName || ''} onChange={e => setProfile(p => p ? ({ ...p, firstName: e.target.value }) : p)} />
                    </div>
                    <div className="acc-field">
                      <label className="acc-label">Last Name</label>
                      <input className="acc-input" value={profile.lastName || ''} onChange={e => setProfile(p => p ? ({ ...p, lastName: e.target.value }) : p)} />
                    </div>
                    <div className="acc-field">
                    ...
                    <div className="acc-field">
                      ...
                      <input className="acc-input" value={profile.phone || ''} onChange={e => setProfile(p => p ? ({ ...p, phone: e.target.value }) : p)} placeholder="(555) 123-4567" />
                    </div>
                  </div>

                  <div className="acc-stats-row">
                    <div className="acc-stat"><span className="acc-stat-num">{profile.orderCount ?? 0}</span><span className="acc-stat-label">Orders</span></div>
                    <div className="acc-stat"><span className="acc-stat-num">{fmt(profile.totalSpent ?? 0)}</span><span className="acc-stat-label">Total Spent</span></div>
                  </div>
                    ...
                    {orders.map(o => (
                      <Link key={o.id} href={`/account/orders/${o.id}?store=${sq}`} className="acc-order-card">
                        ...
                        <div className="acc-order-meta">
                          ...
                          <span className="acc-fulfillment-type"><FulfillmentIcon type={o.fulfillmentType || 'delivery'} /> {o.fulfillmentType === 'pickup' ? 'Pickup' : 'Delivery'}</span>
                          <span className="acc-order-total">{fmt(o.grandTotal ?? 0)}</span>
                        </div>
