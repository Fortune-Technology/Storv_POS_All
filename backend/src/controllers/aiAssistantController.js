...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        search: {
          type: 'string',
          description: 'Optional product name or UPC to search for.',
        },
        low_stock_only: {
          type: 'boolean',
          description: 'If true, return only products below their low-stock threshold. Default false.',
        },
        limit: {
          type: 'number',
          description: 'Max number of products to return. Default 20, max 50.',
        },
      },
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        status: {
          type: 'string',
          enum: ['complete', 'refund', 'voided', 'all'],
          description: 'Filter by status. Default complete.',
        },
      },
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        date_from: {
          type: 'string',
          description: 'Start date YYYY-MM-DD (inclusive). Default 7 days ago.',
        },
        date_to: {
          type: 'string',
        ...
        min_amount: {
          type: 'number',
          description: 'Minimum grand total.',
        },
        max_amount: {
          type: 'number',
          description: 'Maximum grand total.',
        },
        tender_method: {
          type: 'string',
          enum: ['cash', 'card', 'ebt', 'check', 'credit', 'debit'],
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        days: { type: 'number', description: 'Number of days back to include. Default 30. Max 90.' },
      },
    },
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        days: { type: 'number', description: 'Number of days back. Default 7. Max 90.' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        days_ahead: { type: 'number', description: 'How many days to forecast. Default 7. Max 30.' },
      },
    },
  },
  {
...
**Only fall back to a text walkthrough** when the topic doesn't match any tour — e.g., "walk me through configuring promotions" (no promotions tour yet) → use the KB article path with clickable portal links.`,
    input_schema: {
      ...
      properties: {
        slug: {
          type: 'string',
          enum: ['add-product', 'set-age-verification', 'invite-user', 'configure-receipt-printer', 'setup-fuel-type'],
          description: 'The tour slug. Must match one of the available tours exactly.',
        },
      },
      required: ['slug'],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        subject: {
          type: 'string',
          description: 'Short subject line (under 120 chars) summarizing the issue. E.g., "Receipt printer not responding after USB reconnect".',
        },
        body: {
          type: 'string',
          description: 'Detailed body describing what the user was trying to do, what happened, and any details that would help support diagnose. Write in third person ("The user is trying to..."). No code, no SQL.',
        },
        priority: {
          type: 'string',
          enum: ['low', 'normal', 'high', 'urgent'],
          description: 'Urgency. Default normal. Use high for bugs blocking day-to-day operations, urgent for complete outages.',
        },
      },
...
async function execTool(name, input, req) {
  ...
  // error — "I need you to pick an organization first" — rather than
...
async function toolLotterySummary(input, orgId, storeId, req) {
  ...
  const where = { orgId, createdAt: { gte: from, lte: to } };
...
async function toolFuelSummary(input, orgId, storeId, req) {
...
async function toolEmployeeHours(input, orgId, storeId, req) {
  ...
  for (const u of byUser.values()) {
    ...
    let totalMs = 0, currentIn = null, active = false;
    for (const e of u.events) {
      if (e.type === 'in') currentIn = e.createdAt;
      else if (e.type === 'out' && currentIn) {
        totalMs += new Date(e.createdAt) - new Date(currentIn);
        currentIn = null;
      }
    }
    if (currentIn) { active = true; totalMs += new Date() - new Date(currentIn); }
    result.push({
      userId: u.userId,
      name: u.name,
...
async function toolEndOfDayReport(input, orgId, storeId, req) {
  if (!(await userHasPermission(req, 'reports.view'))) {
  ...
  const where = { orgId, createdAt: { gte: from, lte: to }, status: { in: ['complete', 'refund'] } };
  if (storeId) where.storeId = storeId;
  ...
  for (const t of txs) {
    ...
    if (Array.isArray(t.tenderLines)) {
      for (const line of t.tenderLines) {
        const raw = (line.method || '').toLowerCase().trim();
...
async function toolSalesPredictions(input, orgId, storeId, req) {
  ...
  try {
    const { runHoltWinters } = await import('../utils/predictions.js');
...
async function toolLookupCustomer(input, orgId, req) {
...
async function toolVendorOrderSuggestions(input, orgId, storeId, req) {
  ...
  const urgencyFilter = ['critical','high','medium','low'].includes(input.urgency) ? input.urgency : null;

  // Try to use the order engine if available — otherwise derive a simple
  // "below reorder point" list from StoreProduct.
  let suggestions = [];
  try {
    const engine = await import('../services/orderEngine.js');
...
async function toolStartProductTour(input, orgId, req) {
  ...
  // Return a marker the widget can detect (via the toolCalls trace) so it
  // surfaces a prominent "Start guided tour" button. The widget then fetches
  // the full tour via the public endpoint when the user clicks.
  return {
    success: true,
    tour: {
      id: tour.id,
...
async function toolCreateSupportTicket(input, orgId, req) {
  const { subject, body, priority = 'normal' } = input || {};
...
async function toolStoreSummary(input, orgId, storeId, req) {
  ...
  for (const t of txs) {
    ...
    if (!isRefund && Array.isArray(t.lineItems)) {
      for (const li of t.lineItems) {
        if (li.isLottery || li.isFuel || li.isBagFee || li.isBottleReturn) continue;
        const key = li.productId || li.upc || li.name;
        if (!key) continue;
        const prev = productTally.get(key) || { name: li.name, qty: 0, revenue: 0 };
...
async function toolInventoryStatus(input, orgId, storeId, req) {
  ...
  const where = { orgId, deleted: false, active: true };
  if (search) {
    where.OR = [
  ...
  const include = storeId
    ? { storeProducts: { where: { storeId }, select: { quantityOnHand: true, lowStockThreshold: true, inStock: true }, take: 1 } }
    : {};
  ...
  let mapped = products.map(p => {
    ...
    return {
      ...
      quantityOnHand: sp?.quantityOnHand ?? null,
      lowStockThreshold: sp?.lowStockThreshold ?? null,
      inStock: sp?.inStock ?? null,
      isLow: sp?.quantityOnHand != null && sp?.lowStockThreshold != null
        ? Number(sp.quantityOnHand) <= Number(sp.lowStockThreshold) : null,
    };
  });
...
async function toolRecentTransactions(input, orgId, storeId, req) {
  ...
  return {
    ...
    transactions: txs.map(t => ({
      ...
      tenderMethod: t.tenderMethod || (Array.isArray(t.tenderLines) ? t.tenderLines[0]?.method : null),
...
async function toolSearchTransactions(input, orgId, storeId, req) {
  if (!(await userHasPermission(req, 'transactions.view'))) {
    return { error: 'You do not have permission to view transactions.' };
  }
  ...
  const to = input.date_to
    ? new Date(input.date_to + 'T23:59:59.999Z')
    : (() => { const d = new Date(); d.setHours(23,59,59,999); return d; })();

  const where = { orgId, createdAt: { gte: from, lte: to } };
  if (storeId) where.storeId = storeId;
  if (input.min_amount != null) where.grandTotal = { ...(where.grandTotal || {}), gte: Number(input.min_amount) };
  if (input.max_amount != null) where.grandTotal = { ...(where.grandTotal || {}), lte: Number(input.max_amount) };
  if (input.tender_method) where.tenderMethod = input.tender_method;
...
async function buildContext(req) {
  let storeName = null, orgName = null;
  try {
    if (req.storeId) {
...
async function runToolLoop(conversation, userText, req) {
  if (!anthropic) {
  ...
  // and inject them into the system prompt. Safe-failures to [] if OpenAI is
  // unavailable or no articles meet the threshold.
  const kbArticles = await searchKB(userText, { orgId: req.orgId, limit: 3, threshold: 0.35 }).catch(() => []);
  const kbBlock = formatKBForPrompt(kbArticles);
  ...
  let finalText = '';
  let totalTokens = 0;
  let ticketIdCreated = null;

  // Prompt caching — the system prompt + tool definitions are stable across
  // requests (KB block is per-query but appended to the end of the prompt
  // as a separate text block). Mark each with `cache_control: ephemeral`
  // to hit the cache. First request in a 5-minute window pays full cost;
  // subsequent requests get ~90% input cost reduction on the cached blocks.
  // Caching requires min 1024 input tokens for Sonnet — our system prompt
  // + tools easily exceeds that.
  const systemBlocks = [
    {
      type: 'text',
      text: systemPrompt,
  ...
  // Tool defs are cached by placing cache_control on the last tool — Anthropic
  // hashes everything up to that marker.
  ...
  for (let iter = 0; iter < MAX_TOOL_ITERATIONS; iter++) {
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      for (const block of response.content) {
        ...
        try {
          output = await Promise.race([
            execTool(block.name, block.input || {}, req),
            new Promise((_, rej) => setTimeout(() => rej(new Error('Tool timeout')), TOOL_TIMEOUT_MS)),
          ]);
        } catch (err) {
          output = { error: err.message || 'Tool execution failed' };
        }

        // Capture the ticket id if this tool call created one — links the
        // AiMessage row to the SupportTicket for cross-reference.
        if (block.name === 'create_support_ticket' && output?.ticketId) {
      ...
      continue; // loop again — Claude now has tool results to reason with
    }
...
export const sendMessage = async (req, res, next) => {
  try {
    ...
    // Run the tool loop.
    let finalText, toolCallsTrace, totalTokens, ticketIdCreated;
    try {
      ({ finalText, toolCallsTrace, totalTokens, ticketIdCreated } = await runToolLoop(conv, content.trim(), req));
    } catch (err) {
      ...
      // Return 200 so the friendly `assistantMessage` flows through the normal
      // success path in the widget instead of being swallowed by axios's 5xx
      // throw behavior. The `success: false` flag + `error` field preserve
      // the failure signal for any caller that wants to distinguish.
      return res.json({
        success: false,
        error: err.message,
        userMessage: userMsg,
    ...
    const assistantMsg = await prisma.aiMessage.create({
      data: {
        ...
        role: 'assistant',
        content: finalText,
        toolCalls: toolCallsTrace.length ? toolCallsTrace : null,
...
export const submitFeedback = async (req, res, next) => {
  try {
    ...
    if (!['helpful', 'unhelpful', null].includes(feedback)) {
      return res.status(400).json({ error: 'feedback must be "helpful", "unhelpful", or null' });
    }
    ...
    // Auto-escalation: if 👎 + note, enqueue an AiFeedbackReview row for admins
    // to curate into the KB. We use upsert (by messageId) so repeated edits to
    // the note update the same review row instead of creating duplicates.
    if (feedback === 'unhelpful' && noteTrimmed) {
      // Grab the preceding user message as the "question" context.
      ...
      await prisma.aiFeedbackReview.upsert({
        ...
        update: {
          userSuggestion: noteTrimmed,
          // Keep status if admin already triaged; only revert if still pending.
          status: undefined,
        },
      });
    }
...
export const escalateConversation = async (req, res, next) => {
  try {
    ...
    // Tack a system-style assistant message onto the conversation so the user
    // sees the ticket was filed without a page refresh.
...
export const listReviews = async (req, res, next) => {
  try {
    ...
    // Enrich with the linked KB article title (if promoted) in a second query —
    // cheap because admin pages are infrequent.
    const articleIds = reviews.map(r => r.articleId).filter(Boolean);
...
export const promoteReview = async (req, res, next) => {
  try {
    const { title, content, category = 'how-to', tags = [] } = req.body;
    ...
    if (review.status !== 'pending') {
      return res.status(400).json({ error: `Review is already ${review.status}` });
    }
    ...
    if (!embedding) {
      return res.status(503).json({ error: 'Embedding service unavailable. Cannot promote without an embedding.' });
    }

    const article = await prisma.aiKnowledgeArticle.create({
      data: {
        orgId:       req.user.role === 'superadmin' ? null : req.orgId,  // platform-wide if superadmin
        category,
        title:       title.trim(),
...
export const dismissReview = async (req, res, next) => {
  try {
    const review = await prisma.aiFeedbackReview.findUnique({ where: { id: req.params.id } });
    if (!review) return res.status(404).json({ error: 'Review not found' });
    if (review.status !== 'pending') {
...
export const listKbArticles = async (req, res, next) => {
  try {
    const { category, active, source, search, limit = 100 } = req.query;
    const where = {};
    if (req.user.role !== 'superadmin') {
      // Org admins see their own-org + platform-wide articles.
...
export const createKbArticle = async (req, res, next) => {
  try {
    const { title, content, category = 'how-to', tags = [], orgId: orgOverride } = req.body;
    if (!title?.trim() || !content?.trim()) {
      return res.status(400).json({ error: 'title and content are required' });
    }
    const { generateEmbedding } = await import('../services/kbService.js');
    const embedding = await generateEmbedding(`${title}\n\n${content}`);
    if (!embedding) {
      return res.status(503).json({ error: 'Embedding service unavailable. Cannot create article.' });
    }
    const article = await prisma.aiKnowledgeArticle.create({
      data: {
        // Superadmin can pick null (platform-wide) or any orgId; others are locked to their org.
...
export const updateKbArticle = async (req, res, next) => {
  try {
    ...
    if (data.title !== undefined || data.content !== undefined) {
      ...
      if (!embedding) {
        return res.status(503).json({ error: 'Embedding service unavailable. Cannot update.' });
      }
...
export const deleteKbArticle = async (req, res, next) => {
  try {
    ...
    // Soft delete — flip active=false. Hard delete reserved for superadmin.
    if (existing.source === 'seed' && req.user.role !== 'superadmin') {
      return res.status(403).json({ error: 'Seed articles can only be hard-deleted by superadmin.' });
    }
...
/**
 * Public read — any authenticated user with ai_assistant.view can list all
 * active tours available to them (platform-wide + their org). Used by the
 * widget's "Browse tours" panel to let users launch tours without relying
 * on the AI to call the tool.
 */
export const listPublicTours = async (req, res, next) => {
  try {
    ...
    // Only send a step count, not the full steps array (slim payload for list).
    res.json({
      success: true,
      tours: tours.map(t => ({
...
/**
 * Public read — any authenticated user with ai_assistant.view can fetch a tour
 * by slug. Returns full step array so the TourRunner can drive the walkthrough.
 */
export const getTourBySlug = async (req, res, next) => {
  try {
    ...
    if (!tour) return res.status(404).json({ error: 'Tour not found or inactive' });
...
export const createTour = async (req, res, next) => {
  try {
    const { slug, name, description, category = 'onboarding', triggers = [], steps = [] } = req.body;
    ...
    if (err.code === 'P2002') {
      return res.status(409).json({ error: 'A tour with this slug already exists.' });
    }
    next(err);
  }
};
...
export const updateTour = async (req, res, next) => {
  try {
    ...
    const { name, description, category, triggers, steps, active } = req.body;
    const data = {};
    if (name        !== undefined) data.name        = String(name).trim();
    if (description !== undefined) data.description = description?.trim() || null;
    if (category    !== undefined) data.category    = category;
    if (Array.isArray(triggers))   data.triggers    = triggers;
    if (Array.isArray(steps))      data.steps       = steps;
    if (typeof active === 'boolean') data.active    = active;
...
export const getReviewConversation = async (req, res, next) => {
  try {
    const review = await prisma.aiFeedbackReview.findUnique({ where: { id: req.params.id } });
    if (!review) return res.status(404).json({ error: 'Review not found' });
