...
interface LineItem {
  ...
  promoAdjustment?: number | string | null;
}
...
interface DeptOut extends DeptAgg {
  margin: number;
  profit: number;
  department?: string;
  pctTotal?: number;
...
export const getSummaryReport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const where: Prisma.TransactionWhereInput = { orgId, status: 'complete', createdAt: { gte: dateFrom, lte: dateTo } };
    ...
    type TxRow = (typeof txns)[number];

    // Fetch refunds/voids separately
    const voidWhere: Prisma.TransactionWhereInput = { orgId, status: 'voided', createdAt: { gte: dateFrom, lte: dateTo } };
    if (storeId) voidWhere.storeId = storeId;
    const voidCount = await prisma.transaction.count({ where: voidWhere });

    const refundWhere: Prisma.TransactionWhereInput = { orgId, status: 'refund', createdAt: { gte: dateFrom, lte: dateTo } };
    if (storeId) refundWhere.storeId = storeId;
    const refunds = await prisma.transaction.findMany({
      where: refundWhere,
      select: { grandTotal: true },
    });
    const refundTotal = r2(refunds.reduce((s: number, r: { grandTotal: unknown }) => s + Math.abs(Number(r.grandTotal) || 0), 0));
    ...
    const cashierMap: Record<string, { cashierId: string; sales: number; txCount: number }> = {};
    const stationMap: Record<string, { stationId: string; sales: number; txCount: number }> = {};
    const hourlyMap: Record<number, { hour: number; sales: number; count: number }> = {};
    const dailyMap: Record<string, { date: string; sales: number; tax: number; count: number }> = {};
    const tenderMethodMap: Record<string, TenderMethodAgg> = {};
    ...
    for (const tx of txns as TxRow[]) {
      ...
      for (const li of items) {
        ...
        deptMap[dept].cost += (Number(li.costPrice) || (Number(li.unitPrice) || 0) * 0.65) * (Number(li.qty) || 1);
    ...
    // ── Lottery ───────────────────────────────────────────────────────────
    const lotteryWhere: Prisma.LotteryTransactionWhereInput = { orgId, createdAt: { gte: dateFrom, lte: dateTo } };
    ...
    const [cashPayouts, vendorPayments] = await Promise.all([
      ...
      }).catch(() => [] as Array<{ amount: unknown; payoutType: string | null; recipient: string | null }>),
      prisma.vendorPayment.findMany({
        where: { orgId, ...(storeId ? { storeId } : {}), paymentDate: { gte: dateFrom, lte: dateTo } },
        select: { amount: true, paymentType: true, vendorName: true },
      }).catch(() => [] as Array<{ amount: unknown; paymentType: string | null; vendorName: string | null }>),
    ]);
    ...
    const departments: DeptOut[] = Object.values(deptMap).map((d): DeptOut => ({
      ...d,
      sales: r2(d.sales),
      cost: r2(d.cost),
...
export const getEventsReport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const q = req.query as { from?: string; to?: string; type?: string };
    const { from, to, type } = q;
    if (!from || !to) { res.status(400).json({ error: 'from and to required' }); return; }
    ...
    if (req.storeId) where.storeId = req.storeId;
    // Legacy code references `where.type` even though PosLog has no such column.
    // Preserved as-is for runtime back-compat (any caller passing `?type=` would
    // have hit a Prisma error already; without it this is a no-op).
    if (type) (where as unknown as { type: string }).type = type;
    ...
    type EventRow = (typeof events)[number] & { type?: string };

    // Group by type for summary — `type` column doesn't exist on PosLog, so
    // every row falls into 'unknown'. Kept the loop to match legacy response shape.
...
export const getReceiveReport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const q = req.query as { from?: string; to?: string };
    const { from, to } = q;
    if (!from || !to) { res.status(400).json({ error: 'from and to required' }); return; }
    ...
    const summary = {
      ...
      totalItems: (orders as OrderRow[]).reduce((s, o) => s + o.items.length, 0),
      totalValue: r2((orders as OrderRow[]).reduce((s, o) => s + (Number(o.grandTotal) || 0), 0)),
    };
...
export const getHouseAccountReport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const totalBalance = r2((customers as CustomerRow[]).reduce((s, c) => s + (Number(c.balance) || 0), 0));
    const totalLimit = r2((customers as CustomerRow[]).reduce((s, c) => s + (Number(c.balanceLimit) || 0), 0));

    res.json({
      customers: (customers as CustomerRow[]).map((c) => ({
        ...c,
      ...
      summary: {
        ...
        totalBalance,
        totalLimit,
        activeChargeAccounts: (customers as CustomerRow[]).filter((c) => c.instoreChargeEnabled).length,
      },
    });
  } catch (err) { next(err); }
};
