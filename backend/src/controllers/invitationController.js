...
const JWT_TTL   = process.env.JWT_ACCESS_TTL || '2h';
...
async function maybeExpire(inv) {
...
function signToken(userId, extra = {}) {
  return jwt.sign({ id: userId, ...extra }, process.env.JWT_SECRET, { expiresIn: JWT_TTL });
}
...
export const createInvitation = async (req, res, next) => {
  try {
    ...
    const { email, phone, role, storeIds, transferOwnership } = req.body;
    ...
    if (isTransfer) {
      ...
      if (req.role !== 'owner' && req.user?.role !== 'superadmin') {
        return res.status(403).json({ error: 'Only the organisation owner can transfer ownership.' });
      }
      effectiveRole = 'owner';
    } else {
      if (!ASSIGNABLE_ROLES.has(effectiveRole)) {
        return res.status(400).json({ error: `Role "${effectiveRole}" cannot be assigned via invitation.` });
      }
    }
    ...
    const orgName = invitation.organization?.name || 'your organisation';
...
export const getInvitationByToken = async (req, res, next) => {
  try {
    ...
    const fresh = await maybeExpire(inv);
...
export const acceptInvitation = async (req, res, next) => {
  try {
    ...
    const fresh = await maybeExpire(inv);
    if (fresh.status !== 'pending') {
      return res.status(410).json({ error: `Invitation ${fresh.status}.`, status: fresh.status });
    }
    ...
    if (!user) {
      ...
      if (!name || typeof name !== 'string' || name.trim().length < 2) {
        return res.status(400).json({ error: 'Full name is required.', reason: 'name_required' });
      }
      const pwErr = validatePassword(password);
      if (pwErr) return res.status(400).json({ error: pwErr, reason: 'password_invalid' });
    ...
    if (fresh.transferOwnership) {
      ...
      transactionOps.push(
        prisma.userOrg.deleteMany({
          where: { orgId: fresh.orgId, userId: { not: user.id } },
        }),
      );
      // Move the seller's UserStore rows too (remove their store access).
      transactionOps.push(
        prisma.userStore.deleteMany({
          where: { user: { id: { not: user.id } }, store: { orgId: fresh.orgId } },
        }),
      );
      ...
      const stuckSellers = await prisma.user.findMany({
        where:  { id: { not: user.id }, orgId: fresh.orgId },
        select: { id: true, orgs: { where: { orgId: { not: fresh.orgId } }, select: { orgId: true, isPrimary: true } } },
      });
      for (const s of stuckSellers) {
        const otherOrg = s.orgs.find(o => o.isPrimary)?.orgId ?? s.orgs[0]?.orgId ?? null;
        transactionOps.push(
    ...
    // current primary org.
    const existingMemberships = await prisma.userOrg.count({ where: { userId: user.id } });
    transactionOps.push(
      prisma.userOrg.upsert({
        where:  { userId_orgId: { userId: user.id, orgId: fresh.orgId } },
        create: {
          userId:      user.id,
          orgId:       fresh.orgId,
          role:        fresh.role,
    ...
    if (!fresh.transferOwnership && fresh.storeIds.length > 0) {
      transactionOps.push(
        prisma.userStore.deleteMany({
          where: { userId: user.id, store: { orgId: fresh.orgId } },
        }),
      );
      transactionOps.push(
        prisma.userStore.createMany({
          data:           fresh.storeIds.map(storeId => ({ userId: user.id, storeId })),
          skipDuplicates: true,
        }),
      );
    }
    ...
    if (fresh.transferOwnership) {
      outgoingOwners = await prisma.userOrg.findMany({
        where:  { orgId: fresh.orgId, userId: { not: user.id }, role: 'owner' },
    ...
    try {
      ...
      console.warn('[Invitation] post-accept notify failed:', e.message);
    }

    const jwtToken = signToken(user.id, {
      name: user.name, email: user.email, role: fresh.role, orgId: fresh.orgId,
    });
...
export const resendInvitation = async (req, res, next) => {
  try {
    if (!req.orgId) return res.status(403).json({ error: 'No organisation context.' });
...
export const revokeInvitation = async (req, res, next) => {
  try {
    ...
    if (inv.status !== 'pending') {
      return res.status(400).json({ error: `Cannot revoke a ${inv.status} invitation.` });
    }
