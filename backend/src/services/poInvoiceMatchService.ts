...
/** Invoice line item — Prisma stores `lineItems` as a JSON column with vendor-specific
 *  shapes; we treat it loosely with hand-typed fields the matcher reads. */
...
export type VarianceFlag = 'none' | 'minor' | 'major';
...
export interface MatchInvoiceResult {
  matchedPO: { id: string; poNumber: string; vendorName?: string | null } | null;
  matchedItems: MatchedItem[];
  unmatchedInvoiceItems: Array<InvoiceLineItem & { _idx: number }>;
...
export async function matchInvoiceToPO(
  ...
  const invoiceItems: InvoiceLineItem[] = Array.isArray(invoice.lineItems)
  ...
  for (const v of vendors) {
    ...
    if (Array.isArray(v.aliases) && v.aliases.some((a: string) => a.toLowerCase() === vendorName)) { vendorId = v.id; break; }
  }
  ...
  type OpenPORow = Prisma.PurchaseOrderGetPayload<{
    include: {
      items: { include: { product: { select: { id: true; name: true; upc: true } } } };
      vendor: { select: { id: true; name: true } };
    };
  }>;
  const openPOs: OpenPORow[] = await prisma.purchaseOrder.findMany({
    where: poWhere,
    include: {
      items: { include: { product: { select: { id: true, name: true, upc: true } } } },
      vendor: { select: { id: true, name: true } },
  ...
  const poItemPool: PoItemPoolEntry[] = [];
  for (const po of openPOs) {
    for (const item of po.items) {
      poItemPool.push({
        ...
        poItemId: item.id,
        masterProductId: item.masterProductId != null ? String(item.masterProductId) : null,
  ...
  const unmatchedInvoiceItems: Array<InvoiceLineItem & { _idx: number }> = [];
  const costVariances: CostVariance[] = [];
  ...
  for (let idx = 0; idx < invoiceItems.length; idx++) {
    ...
    let bestMatch: PoItemPoolEntry | null = null;
    let bestScore = 0;
    ...
    if (bestMatch && bestScore >= 0.5) {
      ...
      const variancePct = bestMatch.unitCost > 0
        ...
        : 0;
      const varianceFlag: VarianceFlag = variancePct < 5 ? 'none' : variancePct < 15 ? 'minor' : 'major';
  ...
  const poMatchCounts: Record<string, number> = {};
