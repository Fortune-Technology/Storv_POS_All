/**
 * rjr.js — RJR / RAI ack-file parser (EDLP / ScanData / VAP).
 *
...
const slice = (s, start, len) => String(s || '').slice(start, start + len);
const trim  = (s) => String(s || '').trim();

export function parseAck(content, fileName = null, mfrCode = 'rjr_edlp') {
  const result = emptyResult(mfrCode);
  result.fileName = fileName;
  result.batchAccepted = null;
  ...
  for (const raw of lines) {
    ...
    if (recordType === 'R') {
      ...
      const code     = trim(slice(raw, 28, 8)) || null;
      const reason   = trim(slice(raw, 36, 100)) || null;

      result.lines.push({
        recordRef: buildRecordRef(txNumber, upc),
        status,
        reason: status === 'accepted' ? null : reason,
        code:   status === 'accepted' ? null : code,
        txNumber,
        upc,
        originalLine: raw,
      });
