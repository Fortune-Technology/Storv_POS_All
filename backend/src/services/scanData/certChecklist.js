/**
 * certChecklist.js — Derives per-enrollment cert state from the DB (Session 49).
 *
 ...
 *
 * Steps are mfr-agnostic — the playbook text is per-mfr in certPlaybook.js.
 *
 * Returned shape:
 *   {
 *     enrollment: { id, status, environment, mfrRetailerId, ... },
 *     manufacturer: { id, code, name, shortName, parentMfrCode },
 *     overallProgress: 0–100,
 *     readyToActivate: bool,
 *     steps: [
 *       { key, label, status: 'done'|'pending'|'warning', detail?, action?: { label, hint } },
 *       …
 *     ],
 *     stats: { mappingCount, brandFamiliesCovered, lastSubmissionAt, lastAckAt, recentRejectedCount }
 *   }
 */
...
export async function getChecklist({ orgId, enrollmentId }) {
  ...
  steps.push({
    ...
    detail: ackedSubmission
      ? `Last ack: ${new Date(ackedSubmission.ackedAt).toLocaleString()} — ${ackedSubmission.acceptedCount}✓ / ${ackedSubmission.rejectedCount}✗`
  ...
  const readyToActivate = steps.slice(0, 9).every(s => s.status === 'done');
