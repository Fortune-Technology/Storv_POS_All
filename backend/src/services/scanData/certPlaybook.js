/**
 * certPlaybook.js — Per-mfr cert guide content (Session 49).
 *
...
const PLAYBOOKS = {
  ...
  rjr: {
    ...
    commonRejects: [
      { code: 'COLUMN_OFFSET', meaning: 'Field starts at wrong column', fix: 'The fixed-width formatter has byte positions documented in rjrEdlp.js header. If RJR\'s spec was updated, edit the rjrEdlp.js padding widths to match.' },
      { code: 'AMT_FORMAT', meaning: 'Amount field not zero-padded cents', fix: 'All amounts must be cents zero-padded (e.g. $1.99 → 00000199). Check fixedAmt() in formatters/common.js.' },
    ...
    notes:
      'RJR\'s ack format is also fixed-width. The rjr.js parser reads from byte positions documented in its file header. If RJR\'s ack spec is updated, the slicing offsets need to be adjusted.',
  },
};
 ...
 */
export function getPlaybook(mfrCode) {
...
export function listAvailablePlaybooks() {
  return Object.values(PLAYBOOKS).map(p => ({
    code: p.code, name: p.name, estimatedDuration: p.estimatedDuration,
  }));
}
