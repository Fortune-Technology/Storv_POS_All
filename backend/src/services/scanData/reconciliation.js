/**
 * reconciliation.js — Match ack lines back to original tx + redemptions (Session 48).
 *
 * Takes a parsed ack (from ackParsers/index.js) + the submission row it
 * applies to, and:
 ...
 *
 * Args:
 *   submission   — ScanDataSubmission row (must include manufacturerId, periodStart, periodEnd, orgId, storeId)
 *   ack          — parsed ack result from ackParsers/index.js
 *   options:
 *     skipEmail  — bool, suppress notification (used for synthetic-ack tests)
 *
 * Returns: {
 *   submissionId, status, acceptedCount, rejectedCount, warningCount,
 *   redemptionsAccepted, redemptionsRejected, redemptionsUntouched,
 *   notification: { sent, reason },
 * }
 */
export async function reconcileAck({ submission, ack, options = {} }) {
  ...
  await prisma.scanDataSubmission.update({
    ...
    data: {
      ...
      ackFileName:   ack.fileName || submission.ackFileName,
      acceptedCount,
      rejectedCount,
      ackedAt:       new Date(),
      status:        nextStatus,
  ...
  const txIds = [...new Set(redemptionsRaw.map(r => r.transactionId).filter(Boolean))];
  ...
  const txNumberById = Object.fromEntries(txs.map(t => [t.id, t.txNumber]));

  // Build lookup keyed by `${txNumber}|${upc}` → redemptionId
  const redemptionByRef = new Map();
  for (const r of redemptionsRaw) {
    const key = `${txNumberById[r.transactionId] || ''}|${r.qualifyingUpc || ''}`;
  ...
  let notification = { sent: false, reason: null };
  if (rejectedCount > 0 && !options.skipEmail) {
    notification = await notifyOnRejection({ submission, ack }).catch(err => ({
      sent: false, reason: err.message,
    }));
  }
 ...
 */
async function notifyOnRejection({ submission, ack }) {
