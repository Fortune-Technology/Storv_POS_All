/**
 * fuelInventory.js — core inventory + FIFO service for the fuel module.
 *
...
export async function getTankLevel(tankId, tx = prisma) {
...
export async function getAllTankLevels(storeId, tx = prisma) {
...
export async function resolveTankForSale({ orgId, storeId, fuelTypeId, gallons, pumpId }, tx = prisma) {
  ...
  if (pumpId) {
    ...
    const overrideTankId = pump?.tankOverrides && typeof pump.tankOverrides === 'object'
      ? pump.tankOverrides[fuelTypeId]
      : null;
    if (overrideTankId) {
  ...
  if (blend) {
    ...
    return {
      mode: 'blend',
      legs: [
        ...(baseTank    ? [{ tankId: baseTank.id,    gallons: baseGal,    label: 'base'    }] : []),
        ...(premiumTank ? [{ tankId: premiumTank.id, gallons: premiumGal, label: 'premium' }] : []),
      ],
    };
  }
...
async function pickTankForGrade(orgId, storeId, fuelTypeId, tx) {
...
export async function drawFromTank(tankId, gallons, tx = prisma) {
...
export async function applySale({ orgId, storeId, fuelTypeId, gallons, pumpId }, tx = prisma) {
...
export async function applyRefund({ fifoLayers, tankId, gallons }, tx = prisma) {
...
export async function getExpectedLevel(tankId, tx = prisma) {
...
export async function checkDeliveryCostVariance({ orgId, storeId, fuelTypeId, newPricePerGallon }, tx = prisma) {
...
export async function recordDelivery({ orgId, storeId, deliveryDate, supplier, bolNumber, notes, createdById, items }) {
...
export async function recordStickReading({ orgId, storeId, tankId, actualGallons, shiftId, notes, createdById }) {
  return prisma.$transaction(async (tx) => {
