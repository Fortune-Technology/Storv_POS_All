...
import { generateOrderSuggestions, nextPONumber } from './orderEngine.js';
 ...
 */
export async function runAutoOrders() {
  ...
  for (const store of stores) {
    try {
      ...
      const { suggestions, vendorGroups } = await generateOrderSuggestions(store.orgId, store.id);
      ...
      for (const group of vendorGroups) {
        ...
        const items = group.items.map(item => ({
          ...
          qtyOrdered: item.orderUnits || item.orderQty || 1,
          ...
          avgDailySales: item.avgDailySales || item.avgDaily,
      ...
      console.error(`[OrderScheduler] Error for store ${store.name}:`, err.message);
    }
  }
...
export function startOrderScheduler() {
  ...
  const checkAndRun = () => {
    ...
    if (now.getHours() === 6 && now.getMinutes() === 0) {
      runAutoOrders().catch(err => console.error('[OrderScheduler] Failed:', err.message));
    }
  };
