...
export interface SingleRehostResult {
  status: 'ok' | 'exists' | 'failed' | 'skipped' | 'error';
  filename?: string;
  reason?: string;
  url?: string;
  size?: number;
...
export interface BatchRehostResult {
  processed: number;
  succeeded: number;
  failed: number;
  skipped: number;
 ...
 */
export function buildRehostedUrl(filename: string): string {
 ...
 */
export async function rehostSingleImage(row: RehostRow | null | undefined): Promise<SingleRehostResult> {
 ...
 * `batchSize`: how many to process in one run (default 100).
 */
export async function rehostBatch(batchSize: number = 100): Promise<BatchRehostResult> {
  ...
  for (const row of rows) {
    const result = await rehostSingleImage(row as unknown as RehostRow);
 ...
 */
export async function getRehostStatus(): Promise<RehostStatus> {
