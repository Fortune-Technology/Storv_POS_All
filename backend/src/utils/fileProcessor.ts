...
export type TransformFn = (row: Row) => TransformResult;
 ...
 */
export async function parseExcelFile(filePath: string, limit = 0): Promise<Row[]> {
 ...
 * @returns {Promise<Object[]>} - Array of row objects
 */
export async function parseCSVFile(filePath: string, limit = 0): Promise<Row[]> {
...
export async function streamProcessCSV(
    ...
    return new Promise<ProcessSummary>((resolve, reject) => {
 ...
 */
export async function parseFile(filePath: string, limit = 0): Promise<Row[]> {
