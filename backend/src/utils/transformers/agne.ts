...
export function transformRow(
    ...
    const getValue = (name: string): unknown => getColumnValue(row, name);
    ...
    const productCodeKey = String(transformedRow['Product Code'] || '');
    const deptResult = preserveDepartmentID(
        getValue('Department'),
        productCodeKey ? options.originalData?.[productCodeKey]?.Department : undefined,
    );
    ...
    const dm = depositMapping as Record<string, unknown>;
    if (existingDeposit && dm) {
        const existingDepositKey = String(existingDeposit);
        if (dm[existingDepositKey]) {
            transformedRow['Fee ID'] = dm[existingDepositKey];
        } else {
            const normalizedDeposit = parseNumeric(existingDeposit);
            if (normalizedDeposit !== null && dm[normalizedDeposit.toString()]) {
                transformedRow['Fee ID'] = dm[normalizedDeposit.toString()];
            } else {
                const itemKey = String(transformedRow.UPC || transformedRow['Product Code'] || '');
                if (dm[itemKey]) {
                    transformedRow['Fee ID'] = dm[itemKey];
                } else {
