 ...
 */
function padUPC(upc: unknown): string {
 ...
 */
function formatDateDDMMYYYY(dateStr: unknown): { value: string; warning: string | null } {
    ...
    // helpers
    const toFourDigitYear = (yy: string): string | null => {
        const n = parseInt(yy, 10);
        if (Number.isNaN(n)) return null;
        return (n < 50) ? `20${String(n).padStart(2, '0')}` : `19${String(n).padStart(2, '0')}`;
    };

    const pad2 = (v: string | number): string => String(v).padStart(2, '0');
