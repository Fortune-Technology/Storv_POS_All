 ...
 */
export const applyDOWFactors = (forecast: number[], startDate: Date | string | number): number[] => {
 ...
 */
export const buildPredictionTimeline = (
 ...
 */
export const computeWeatherImpact = (historicalData: HistoricalRow[] | null | undefined): WeatherImpact => {
  ...
  const coldDays  = valid.filter((d) => (d.tempMean ?? 0) < 32);
  const hotDays   = valid.filter((d) => (d.tempMean ?? 0) > 90);
  const snowDays  = valid.filter((d) => (d.weatherCode ?? 0) >= 71 && (d.weatherCode ?? 0) <= 77);
  const normalDays = valid.filter((d) => (d.precipitation || 0) <= 0.5 && (d.tempMean ?? 0) >= 32 && (d.tempMean ?? 0) <= 90);

  const normalAvg = normalDays.length > 3
    ? normalDays.reduce((s, d) => s + d.sales, 0) / normalDays.length
    : avgSales;

  const factor = (days: HistoricalRow[]): number => {
 ...
 */
type LooseRow = { date: string; predicted?: number; [k: string]: unknown };

export const applyWeatherToPredictions = <T extends LooseRow>(
  predictions: T[],
  weatherForecast: WeatherForecastRow[] | null | undefined,
  impact: WeatherImpact,
): T[] => {
  const weatherMap: Record<string, WeatherForecastRow> = {};
  for (const w of (weatherForecast || [])) weatherMap[w.date] = w;

  return predictions.map((p) => {
    const pp = p as PredictionRow;
    const w = weatherMap[p.date];
    if (!w) return { ...p, weatherAdjusted: pp.predicted, factors: { ...(pp.factors || {}) } } as T;

    let multiplier = 1.0;
    const factors: Record<string, unknown> = { ...(pp.factors || {}) };
    ...
    // Snow impact
    if ((w.weatherCode ?? 0) >= 71 && (w.weatherCode ?? 0) <= 77) {
    ...
    return {
      ...p,
      weatherAdjusted: Math.max(0, Math.round((pp.predicted ?? 0) * multiplier * 100) / 100),
 ...
 */
export const applyHolidayFactors = <T extends LooseRow>(predictions: T[]): T[] => {
  return predictions.map((p) => {
    const pp = p as PredictionRow;
    if (!pp.isHoliday || !pp.holidayName) return p;
    const mult = HOLIDAY_MULTIPLIERS[pp.holidayName] ?? 0.85;
    const factors = { ...(pp.factors || {}), holiday: { label: pp.holidayName, impact: mult - 1 } };
    const predicted = Math.max(0, Math.round((pp.predicted ?? 0) * mult * 100) / 100);
    return {
      ...p,
      predicted,
      factors,
...
interface HourlyTx {
  ...
  grandTotal: number | string | { toNumber?: () => number };
}

export const computeHourlyDistribution = (transactions: HourlyTx[]): number[] => {
 ...
 */
export const breakIntoHourly = (
  dailyPrediction: number,
  hourlyDistribution: number[],
): Array<{ hour: number; label: string; predicted: number; pct: number }> => {
 ...
 */
interface MonthlyAgg { total: number; days: number }

export const aggregateToMonthly = (
  dailyPredictions: Array<{ date: string; predicted?: number; weatherAdjusted?: number; [k: string]: unknown }>,
): Array<{ month: string; predicted: number; days: number; avgDaily: number }> => {
  const months: Record<string, MonthlyAgg> = {};
  for (const p of dailyPredictions) {
    const m = p.date.slice(0, 7); // YYYY-MM
    if (!months[m]) months[m] = { total: 0, days: 0 };
    months[m].total += p.weatherAdjusted ?? p.predicted ?? 0;
    months[m].days += 1;
  }
...
export const calculateVelocity = (
  movements: number[] | null | undefined,
): { avgWeekly: number; trend: number; recommendation: 'reorder' | 'ok' | 'overstock' } => {
