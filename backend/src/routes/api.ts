...
router.post('/transform', async (req, res, next) => {
    try {
        ...
        const depositMapping: Record<string, string> = {};
        try {
            ...
            if (fs.existsSync(depositMappingPath)) {
                ...
                (rows as Array<Record<string, string>>).forEach((row) => {
        ...
        setImmediate(async () => {
            try {
                const result = await streamProcessFile(
                    ...
                    outputPath,
                    (row: Record<string, unknown>) => transformRow(row, depositMapping, { vendorId }),
                );
